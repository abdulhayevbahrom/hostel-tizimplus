...
export function SalariesPage({ currentEmployee }) {
  ...
  return <div className="salaries-page">
    ...
    <Modal open={Boolean(selected)} onCancel={closePayment} footer={null} title={selected ? `${fullName(selected.employee)}ga oylik berish` : ''} destroyOnHidden>
    ...
    <Modal open={Boolean(historyRow)} onCancel={() => setHistoryRow(null)} footer={null} width={820} title={historyRow ? `${fullName(historyRow.employee)} — ${period.format('MMMM YYYY')} tarixi` : ''}>
