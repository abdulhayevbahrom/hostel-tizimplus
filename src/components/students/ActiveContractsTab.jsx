...
export function ActiveContractsTab() {
  ...
  const { data, isLoading, isFetching, error } = useGetActiveStudentContractsQuery({ page, ...(deferredSearch ? { search: deferredSearch } : {}), ...(room ? { room } : {}), ...(contractType ? { contractType } : {}) })
