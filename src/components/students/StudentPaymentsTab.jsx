...
export function StudentPaymentsTab({ student, currentEmployee }) {
  ...
  const paymentInstallments = (data?.installments || []).filter((item) => String(item.contract) === selectedContractId)
  const payableInstallments = paymentInstallments.filter((item) => item.paidAmount < item.amount)
