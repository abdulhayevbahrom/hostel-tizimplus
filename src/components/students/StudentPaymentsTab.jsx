...
export function StudentPaymentsTab({ student, currentEmployee }) {
  ...
  const remove = async (id) => {
    try { await deletePayment(id).unwrap(); toast.success('To‘lov o‘chirildi') }
    catch (requestError) { toast.error(apiErrorMessage(requestError)) }
  }
  const openPayment = () => { const contract = contractOptions[0]; const installment = (data?.installments || []).find((item) => String(item.contract) === contract?._id && item.paidAmount < item.amount); paymentForm.setFieldsValue({ contract: contract?._id, installment: installment?._id, amount: null, method: 'cash', note: '' }); setPaymentOpen(true) }
