...
import { Alert, AutoComplete, Button, Checkbox, Form, Input, InputNumber, Modal, Segmented } from 'antd'
...
export function StudentFormModal({ open, student, loading, error, onClose, onSubmit }) {
  ...
  const selectedUniversity = universities.find((item) => item.id === universityId || item.name.toLocaleLowerCase() === String(universityId || '').toLocaleLowerCase())
  const faculties = (facultyData?.faculties || []).filter((item) => (item.university?.id || item.university) === selectedUniversity?.id)
  const prepare = (visible) => {
    if (!visible) return
    form.setFieldsValue(student ? { ...student, studentStatus: student.studentStatus || 'green', hasTaxContract: Boolean(student.hasTaxContract), disciplinaryStatus: student.disciplinaryStatus || 'clear', disabilityStatus: student.disabilityStatus || 'none', university: student.university?.name || '', faculty: student.faculty?.name || '', passport: `${student.passportSeries || ''}${student.passportNumber || ''}` } : initialValues)
    setPhotoFiles([])
  ...
  return (
    <Modal open={open} onCancel={onClose} afterOpenChange={prepare} footer={null} destroyOnHidden width={900} rootClassName="student-form-modal" title={student ? 'Talabani tahrirlash' : 'Yangi talaba qo‘shish'}>
      <Form form={form} layout="vertical" initialValues={initialValues} requiredMark={false} onFinish={async (values) => {
        ...
        try { await onSubmit({ values: { ...studentValues, passportSeries: passport.slice(0, 2), passportNumber: passport.slice(2) }, photoFiles, removePhoto }) }
        ...
        <div className="student-form-grid">
          ...
          <Form.Item name="phone" label="Telefon" rules={[{ required: true, message: 'Telefon raqamini kiriting' }, { pattern: /^\d{9}$/, message: 'Masalan: 939119572' }]}><Input maxLength={9} inputMode="numeric" placeholder="939119572" /></Form.Item>
          ...
          <Form.Item name="parentPhone" label="Ota-onasi telefoni" rules={[{ pattern: /^\d{9}$/, message: 'Masalan: 939119572' }]}><Input maxLength={9} inputMode="numeric" placeholder="939119572" /></Form.Item>
          <Form.Item name="university" label="Universitet" rules={[{ required: true, whitespace: true, message: 'Universitetni tanlang yoki nomini yozing' }]}><AutoComplete filterOption={(input, option) => String(option?.search || '').toLocaleLowerCase().includes(input.toLocaleLowerCase())} placeholder="Qidiring yoki yangi universitet nomini yozing" options={universities.map((item) => ({ value: item.name, label: item.shortName ? `${item.name} (${item.shortName})` : item.name, search: `${item.name} ${item.shortName || ''}` }))} /></Form.Item>
          <Form.Item name="faculty" label="Fakultet" rules={[{ required: true, whitespace: true, message: 'Fakultetni tanlang yoki nomini yozing' }]}><AutoComplete disabled={!universityId} filterOption={(input, option) => String(option?.value || '').toLocaleLowerCase().includes(input.toLocaleLowerCase())} placeholder="Qidiring yoki yangi fakultet nomini yozing" options={faculties.map((item) => ({ value: item.name, label: item.name }))} /></Form.Item>
        ...
        <div className="student-identity-grid">
          <Form.Item name="jshr" label="JSHR" rules={[{ pattern: /^\d{14}$/, message: 'JSHR 14 ta raqamdan iborat bo‘lsin' }]}><Input maxLength={14} inputMode="numeric" placeholder="Ixtiyoriy — 14 xonali JSHR" onBlur={checkIdentity} /></Form.Item>
          <Form.Item name="passport" label="Pasport (ID karta)" rules={[{ pattern: /^[A-Za-z]{2}\s?\d{7}$/, message: 'Masalan: AA1234567' }]}><Input maxLength={10} placeholder="Ixtiyoriy — AA1234567" onInput={(event) => { event.currentTarget.value = event.currentTarget.value.toUpperCase() }} onBlur={checkIdentity} /></Form.Item>
        </div>
        ...
        <div className="student-form-actions"><Button htmlType="submit" loading={loading || submitting} disabled={loading || submitting} className="student-submit-btn">{student ? 'Yangilash' : 'Saqlash'}</Button><Button disabled={loading || submitting} onClick={onClose}>Yopish</Button></div>
      </Form>
    </Modal>
  )
}
